...
class CartProductPercentAction extends CartRuleActionBase
{
    ...
    public function evalDiscount(&$params, $hostObj, &$itemDiscountMap, &$itemDiscountTaxInclMap, $productConditions)
    {
        if (!array_key_exists('cart_items', $params)) {
            throw new ApplicationException('Error applying the "Discount each cart item unit price by percentage of the original price" price rule action: the cart_items element is not found in the action parameters.');
        }
        ...
        foreach ($cartItems as $item) {
            ...
            if ($this->isActiveForProduct($item->product, $productConditions, $currentProductPrice, $ruleParams)) {
                $discountValue = round(($currentProductPrice*$hostObj->discount_amount/100), 2);
                if ($discountValue > $currentProductPrice) {
                    $discountValue = $currentProductPrice;
                }

                $totalDiscount += $discountValue*$item->quantity;
                $itemDiscountMap[$item->key] += $discountValue;

                $totalDiscountInclTax = $discountValue;
                if ($includeTax) {
