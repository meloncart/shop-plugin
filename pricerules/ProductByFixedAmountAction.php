...
class ProductByFixedAmountAction extends RuleActionBase
{
    ...
    public function evalAmount(&$params, $hostObj)
    {
        if (!array_key_exists('current_price', $params)) {
            throw new ApplicationException('Error applying the "By fixed amount" price rule action: the current_price element is not found in the action parameters.');
        }

        $currentPrice = $params['current_price'];

        return max($currentPrice - $hostObj->discount_amount, 0);
    }
}
